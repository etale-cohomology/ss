...
int main(int nargs, char* args[]){  setlocale(LC_NUMERIC,"");  struct stat fs;
  ...
  vec_forall(img_paths, it){
    m_file_t* img_file    = m_file_init(*it);  if(img_magic_get(img_file->data)!=IMG_MAGIC) printf("\x1b[31mERROR  \x1b[0mGot img magic \x1b[91m%016lx\x1b[0m, expected img magic \x1b[94m%016lx\x1b[0m. Are you sure this is a \x1b[35mbi \x1b[0mimg?\n", img_magic_get(img_file->data), IMG_MAGIC);
