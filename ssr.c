...
// ----------------------------------------------------------------------------------------------------------------------------#
m_file_t* img_open(char* imgpath, xcb_screen_t* xcb_screen, m_blosc_t* blosc, i64* img_ndim_w, i64* img_ndim_h, void* out_data){  // @img_ndim_w and @img_ndim_h are `inout` parameters
  m_file_t* img_file = m_file_init(imgpath);
  *img_ndim_w        = img_ndim_w_get(img_file->data);
  *img_ndim_h        = img_ndim_h_get(img_file->data);
  i32 img_depth      = img_depth_get(img_fmt_get(img_file->data));
...
int main(int nargs, char* args[]){  setlocale(LC_NUMERIC,"");  struct stat fs;
  ...
  // ----------------------------------------------------------------------------------------------------------------------------#
#if 1
  m_file_t* img_file    = m_file_init(img_paths[img_idx]);  if(img_magic_get(img_file->data)!=IMG_MAGIC) printf("\x1b[31mERROR  \x1b[0mGot img magic \x1b[91m%016lx\x1b[0m, expected img magic \x1b[94m%016lx\x1b[0m. Are you sure this is a \x1b[35mbi \x1b[0mimg?\n", img_magic_get(img_file->data), IMG_MAGIC);
  ...
  size_t dbytes,cbytes,blocksize; blosc_cbuffer_sizes(img_file->data+IMG_DATA_POS, &dbytes, &cbytes, &blocksize);  printf("blosc \x1b[32m%'lu \x1b[91m/ \x1b[0m%'lu\n", cbytes,dbytes);  //if(dbytes > img_bdim){ printf("\x1b[91mFAIL  \x1b[0mGot img bdim \x1b[91m%'ld\x1b[0m, expected img bdim \x1b[94m%'ld\x1b[0m\n", dbytes, img_bdim); m_exit_fail(); }
  ...
  xcb_create_window(xcb_connection, xcb_screen->root_depth, xcb_window, xcb_screen->root, 0,0,img_ndim_w,img_ndim_h, 0,XCB_WINDOW_CLASS_INPUT_OUTPUT, xcb_screen->root_visual, XCB_CW_BACK_PIXMAP|XCB_CW_EVENT_MASK, (u32[]){XCB_BACK_PIXMAP_NONE, XCB_EVENT_MASK_KEY_PRESS|XCB_EVENT_MASK_EXPOSURE|XCB_EVENT_MASK_STRUCTURE_NOTIFY});  // The values of this array MUST match the order of the enum where all the masks are defined!
  ...
  printf("\x1b[33mss read  \x1b[0mpath \x1b[92m%s  \x1b[0mximg ndim (\x1b[31m%d\x1b[91m;\x1b[32m%d\x1b[0m)  \x1b[0mximg bdim \x1b[94m%'ld  \x1b[0mimg ndim \x1b[0m(\x1b[31m%d\x1b[91m;\x1b[32m%d\x1b[0m)  \x1b[0mimg bdim \x1b[94m%'ld  \x1b[0mimg fmt \x1b[35m%x  \x1b[0mnthreads \x1b[32m%d\x1b[0m/\x1b[35m%ld  \x1b[0mscreen \x1b[35m%d\x1b[0m\n", img_paths[img_idx], xcb_shimg->img->width,xcb_shimg->img->height,xcb_shimg->img->size, img_ndim_w,img_ndim_h,img_bdim, img_fmt, nthreads,sysconf(_SC_NPROCESSORS_ONLN), xcb_screen_idx);
  ...
  img_file = img_open(img_paths[img_idx], xcb_screen, blosc, &img_ndim_w,&img_ndim_h, xcb_shimg->img->data);
  dt_end(&dt1);
  ...
  xcb_generic_event_t* xcb_ev = xcb_wait_for_event(xcb_connection);  // xcb_ev_poll(xcb_connection, 8);  if(xcb_ev==NULL) continue;
    switch(xcb_ev_type(xcb_ev)){
      ...
      case XCB_KEY_PRESS:{
        ...
        dt_end(&dt);  printf("  %-10s", "key->put"); dt_show(&dt);
      }break;
      case XCB_EXPOSE:{
        ...
        dt_end(&dt);  printf("  %-10s", "exp->put"); dt_show(&dt);
      }break;
    }
    free(xcb_ev);
  }
  ...
  // ----------------------------------------------------------------------------------------------------------------------------#
#if 0  // Prepend a header to a headerless image and overwrite the headerless image with the new, headerful image
  m_sep();
  img_header_t header = {magic:IMG_MAGIC, version:0x0000000000000001, ndim_w:1920,ndim_h:1080, fmt:IMG_FMT_BGR8888};  assert(sizeof(img_header_t)==0x40);

  m_fori(i, 0,vec_idim(img_paths)){ i64 st;
    m_file_t* file     = m_file_init(img_paths[i]);
    char*     tmp_path = alloca(strlen(img_paths[i])+2);  snprintf(tmp_path,strlen(img_paths[i])+2, "%sz", img_paths[i]);  printf("\x1b[33mpath out \x1b[92m%s\n\x1b[0m", tmp_path);  // IMPORTANT! You CANNOT read/write to the SAME FILE AT THE SAME TIME! So we must keep a dummy file, and @rename(2) at the end

    int fd = open(tmp_path, O_RDWR|O_CREAT, 0b110000000);  m_chks(fd);
    m_chks(ftruncate(fd, IMG_HEADER_BDIM + file->bdim));
    st=write(fd, &header,       IMG_HEADER_BDIM);  m_chk(-(st!=IMG_HEADER_BDIM));
    st=write(fd, file->data, file->bdim);    m_chk(-(st!=file->bdim));
    m_chks(close(fd));

    m_file_free(file);
    m_chks(rename(tmp_path, img_paths[i]));
  }
#endif

  // ----------------------------------------------------------------------------------------------------------------------------#
#if 0  // Change image format
  m_sep();
  int  fd   = open(img_paths[img_idx], O_RDWR, 0b110000000);  m_chks(fd);
  u64* data = mmap(NULL, fd_bdim(fd), PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
  data[4]   = IMG_FMT_BGR565;  printf("IMG_FMT_BGR565 %d\n", IMG_FMT_BGR565);
  munmap(data, fd_bdim(fd));
  m_chks(close(fd));
#endif
